 ...
 *
 * Term Project - Develop an interactive STM32 text-based RPG simulating survival after escaping a damaged space station.
...
void selectCharacter() {
    ...
    choice = getNumericInput(1, 3);

    if (choice == 1) {
        strcpy(character, "Engineer");
        showEngineerArt();
    } else if (choice == 2) {
        strcpy(character, "Soldier");
        showSoldierArt();
    } else {
        strcpy(character, "Pilot");
        showPilotArt();
    }
}
...
// Function that ask the user to begin descent or launch ship by pressing enter or typing RUN
//-----------------------------------------------------------------------------------------
void beginDescent() {
    char run[10];
    while (1) {
        if (!fgets(run, sizeof(run), stdin)) {
            printf("                Input error. Try again.\n");
            continue;
        }
